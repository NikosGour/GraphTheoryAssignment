...
synthetic_adjacency_matrix_with_threshold_removed = synthetic_adjacency_matrix - best_threshold
...
for i, row in enumerate(edges_that_dont_match):
    for j, col in enumerate(row):
        if col:
            num += 1
            if i != j and synthetic_adjacency_matrix_with_threshold_removed[i][j] >= 0:
                num_of_positives += 1
                coords_of_recommendations.append(([i, j], synthetic_adjacency_matrix_with_threshold_removed[i][j]))

print(f"number of edges that dont match: {num}")
print(f"number of edges that dont match that are also one: {num_of_positives}")

coords_of_recommendations = sorted(coords_of_recommendations, key=lambda x: x[1], reverse=True)
with open("amazon_recommendations.txt", "w") as f:
    for coords, weight in coords_of_recommendations:
        f.write(f"{coords[0]}\t{coords[1]}\t{weight}\n")